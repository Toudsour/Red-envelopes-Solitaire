...
void Game::Start(int Round)
{
    ...
    {
        double Max=PacketMoney/PacketQuota;
        ...
        for(int j=1;j<=MaxPeole;j++)
        {
            ...
            if(Index==1)
            {
                double LeftM=RedPacket.GetLeftMoney();
                double LeftQ=RedPacket.GetLeftQuota();
                if((LeftM/LeftQ)>Max)
                    continue;
            }
        ...
        int Luck=Debug.Statistics(Money,PacketMoney);
